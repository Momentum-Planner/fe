...
/**
 * 체결 한 행. **「건당」이다** — 계획으로 묶지 않는다 (2026-09-13).
 *
 * 💀 설계 문서는 *「한 계획의 매수·매도가 한 줄에 이어진다」* (14장 ①) 였는데,
 * 묶어 놓으니 **세는 단위가 화면에서 두 개**가 됐다 — 위의 통계는 「매도 기록
 * 하나가 한 건」인데 목록은 「계획 하나가 한 줄」이라, 「계획·미달 8건」을
 * 목록에서 세려면 줄이 아니라 줄 «안»을 세어야 했다. **같은 것을 두 번
 * 세지 않는다.**
 *
 * 계획은 지워지지 않고 «칸»으로 남는다 — 줄마다 배지와 이름이 다시 선다.
 */
 ...
 * 「계획 있음 34 / 없음 12」를 읽고 나면 다음 손이 하는 일은 **그 12건을
 * 보는 것**이다. 숫자를 읽는 자리와 고르는 자리를 따로 두지 않는다.
 */
export interface ListFilter {
   ...
   *
   * 💀 **워터폴의 기간과 «따로» 논다** (2026-09-15). 성질이 다르다 —
   * 워터폴의 기간은 *「통계를 어느 구간으로 접을까」* 이고, 여기 날짜는
   * *「그날 무엇을 샀더라」* 를 **찾는** 것이다. 앞은 «달»이면 충분하고
   * 뒤는 «날»이라야 한다. 하나로 묶으면 거래 한 건을 찾으려고 통계 전체를
   * 그 하루로 접어야 한다.
   */
  from: string
  to: string
...
export const isAllFilter = (f: ListFilter) =>
  f.side === 'ALL' && f.plan === 'ALL' && f.gate === 'ALL' && !f.from && !f.to

export const filterRows = (rows: FillRow[], f: ListFilter): FillRow[] =>
  rows.filter(
    ({ rec, planClass }) =>
      (f.side === 'ALL' || rec.side === f.side) &&
      (f.plan === 'ALL' || planClass === f.plan) &&
      (f.gate === 'ALL' || (f.gate === 'PASS') === gatePassed(rec)) &&
      (!f.from || rec.filledAt >= f.from) &&
      (!f.to || rec.filledAt <= f.to),
  )
